...
if [ "$ENV" == "prod" ]; then
  ...
  HDFS_ROOT="/prod/customer_etl"
  HDFS_INPUT="/prod/customer_etl/input"
  HDFS_OUTPUT_ROOT="/prod/customer_etl/output"
  HDFS_OUTPUT="/prod/customer_etl/output/loyalty_snapshot_${RUN_DATE}"
  FINAL_CSV="/opt/spark-apps/shared_output/customer_etl/prod_loyalty_snapshot_${RUN_DATE}.csv"
else
  # Dev HDFS paths (default)
  LANDING_PATH="/opt/spark-apps/landing/customer_etl/"
  HDFS_ROOT="/customer_etl"
  HDFS_INPUT="/customer_etl/input"
  HDFS_OUTPUT_ROOT="/customer_etl/output"
  HDFS_OUTPUT="/customer_etl/output/loyalty_snapshot_${RUN_DATE}"
  FINAL_CSV="/opt/spark-apps/shared_output/customer_etl/dev_loyalty_snapshot_${RUN_DATE}.csv"
fi
